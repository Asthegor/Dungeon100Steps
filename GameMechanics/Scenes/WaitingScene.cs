...
namespace Dungeon100Steps.GameMechanics.Scenes
{
    ...
    public class WaitingScene(SceneManager sceneManager) : Scene(sceneManager)
    {
        ...
        public override void Reset()
        {
            ...
            _currentEvent = ServiceLocator.Get<Event>(ProjectServiceKeys.CurrentEvent);
            if (_currentEvent != null)
            {
                if (_currentEvent.Result == EventResult.Defeat || _currentEvent.Result == EventResult.Fled)
                {
                    SetCurrentScene(_currentEvent.Result == EventResult.Fled ? ProjectSceneKeys.CityScene : ProjectSceneKeys.DefeatScene);
                    ServiceLocator.Unregister(ProjectServiceKeys.CurrentEvent);
                    ServiceLocator.Unregister(ProjectServiceKeys.CurrentDungeon);
                    return;
                }
            }
            _backButton.Content = _dungeon.CurrentLevel < 0 ? "GAME_FIRST_EVENT" : "GAME_NEXT_EVENT";
        ...
        private void GoToNextEvent(Button button)
        {
            ...
            if (nextEvent == null)
            {
                SetCurrentScene(ProjectSceneKeys.VictoryScene);
                return;
            }
            ...
            if (nextEvent is CombatEvent)
                ...
                throw new InvalidOperationException("Événement inconnu dans le donjon.");
        }
        ...
        private void CreateWeaponAndArmorMenu()
        {
            ...
            var weaponMenuItem = _weaponAndArmorMenu.AddItem(font, "INVENTORY_EQUIP", PaletteColors.MenuItem, OnMenuItemSelection, OnMenuItemDeselection, EquipItem);
            _itemMenus[ItemType.Weapon] = weaponMenuItem;
            _itemMenus[ItemType.Armor] = weaponMenuItem;

            _weaponAndArmorMenu.AddItem(font, "INVENTORY_DROP", PaletteColors.MenuItem, OnMenuItemSelection, OnMenuItemDeselection, DropItem);
        ...
        private void CreatePotionMenu()
        {
            ...
            var potionMenuItem = _potionMenu.AddItem(font, "INVENTORY_DRINK", PaletteColors.MenuItem, OnMenuItemSelection, OnMenuItemDeselection, DrinkPotion);
            _itemMenus[ItemType.Potion] = potionMenuItem;

            _potionMenu.AddItem(font, "INVENTORY_DROP", PaletteColors.MenuItem, OnMenuItemSelection, OnMenuItemDeselection, DropItem);
